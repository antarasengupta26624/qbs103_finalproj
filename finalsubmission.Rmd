...
```{r}
# Transposing original gene dataframe
genes_transposed <-as.data.frame(t(genes))
# Getting rid of useless first row
names(genes_transposed) <- genes_transposed[1,]
# Add row names of genes transposed to a participant ID
genes_transposed$participant_id <- row.names(genes_transposed)
genes_transposed <- genes_transposed[-1,]
# Merging new transposed gene matrix with series dataframe
merged2 <- merge(genes_transposed,series,by='participant_id')
```

```{r heatmap}
# Creating a new dataframe with above merged dataframe
newdf <- merged2

# Accessing dataframe only has rows for genes of interest
newdf3 <- as.data.frame(newdf[,20:30])
newdf3 <-  mutate_all(newdf3, function(x) as.numeric(as.character(x)))


# Creating annotations of categorical covariates
annotations <- data.frame(Sex = merged2$sex,DiseaseStatus = merged2$disease_status)
annotation_col <- list(Sex=c(' male' = 'orange',' female'='purple', ' unknown' = 'yellow'),
                          DiseaseStatus= c('disease state: COVID-19'='green','disease state: non-COVID-19'='red'))
unique(annotations$DiseaseStatus)

# Adjusting rownames
row.names(newdf3) <- row.names(merged2)
row.names(annotations) <- row.names(merged2)

# Creating the heatmap
heat <- pheatmap(newdf3,
         show_rownames = FALSE,
         cluster_rows = TRUE,
         cluster_cols = TRUE,
